...
def format_response( res):
    # Removing the load_csv from the answer if it exists.
    csv_line = res.find("read_csv")
    if csv_line > 0:
        return_before_csv_line = res[0:csv_line].rfind("\n")
        if return_before_csv_line == -1:
            # The read_csv line is the first line so there is nothing is needed before it.
            res_before = ""
        else:
...
def get_primer(df_dataset,df_name):
    # Primer function to take a dataframe and its name and the name of the columns and if any columns with less than 20 unique values it adds the values to the primer # and horizontal grid lines and labeling
    ...
    primer_desc = primer_desc + "\nUsing Python create a script using the dataframe df to graph the following: "
...
def ques(question_to_ask, model_type, key):
    if model_type == "gpt-4":
        task = "Explain the python code line by line."
